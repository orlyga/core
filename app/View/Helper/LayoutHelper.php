 ...
 */
	public function nestedLinks($links, $options = array(), $depth = 1) {
		...
		$output = '';
		//orly added translation for menu
		foreach ($links as $link) {
			...
			$linkOutput = $this->Html->link(__($link['Link']['title']), $link['Link']['link'], $linkAttr);
 ...
 */
	public function nodeList($alias, $options = array()) {
		...
		$output = '';
		if(count($this->_View->viewVars['nodes_for_layout'])==0);
 ...
 */
	public function adminMenus($menus, $options = array()) {
		...
		foreach ($sorted as $menu) {
			...
			//orly added support multilangual
			$link = $this->Html->link(__($menu['title']), $menu['url'], $menu['htmlAttributes']);
