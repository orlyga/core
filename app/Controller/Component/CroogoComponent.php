 ...
 */
 //orly added function
 public function getmenu($menuAlias,$controller=null) {
		...
		if (isset($controller)) return $this->menus_for_layout;
	}
 //orly change to support menus inside controller
	public function menus($controller=null) {
		$menus = array();
 ...
 */
	public function nodes() {
		...
		foreach ($nodes as $alias => $options) {
			...
			//orly added
			if(($options['find']=='threaded')&&(isset($node[0]['Node']['id']))){
				$node['children'] = $this->controller->Node->find('all', array(
						'conditions' => array('Node.parent_id'=>$node[0]['Node']['id']),
						'order' => $options['order'],
						'limit' => $options['limit'],
						'cache' => array(
								'prefix' => 'croogo_nodes_' . $alias . '_',
