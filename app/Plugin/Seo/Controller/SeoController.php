 ...
 * @access public
 */
    public $uses = array('Node', 'Vocabulary', 'Seo','Photon.Album','Photon.Photo');
    ...
    function index() {
        ...
        $sitemapData = $this->__getSiteMapData('Node',$this->Node->find('all',array('conditions' => array('Node.status =' => 1))));
...
*/
    ...
    function sitemapxml() {
        ...
        $sitemapData = $this->__getSiteMapData('Node',$this->Node->find('all',array('conditions' => array('Node.status =' => 1))));
        $sitemapAlbumData = $this->__getSiteMapData('Album',$this->Album->find('all', array('conditions' => array('Album.node_id' =>'NULL'))));
        $sitemapPhotoData = $this->__getSiteMapData('Photo',$this->Photo->find('all'));
        $sitemapData = array_merge($sitemapData,$sitemapAlbumData);
        $sitemapData = array_merge($sitemapData, $sitemapPhotoData);
        $this->set(compact('sitemapData'));
        $this->RequestHandler->respondAs('xml');
    }
    
    
     function __getSiteMapData ($model,$data) {        
         $sitemapData = array();
        //debug($nodes);
         if($model=="Album") $pth="/nodes/gallery/album";
         else $pth="/".strtolower($model);
               foreach ($data as $key=>$d) {
	            $sitemapData[$key][$model]['title'] = $d[$model]['title'];
	            if(isset($d[$model]['path']))
	           		 $sitemapData[$key][$model]['path'] = $d[$model]['path'];
	            else {
	            	$path=(isset($d[$model]['slug']))?$d[$model]['slug'] : $d[$model]['id'];
	            	$sitemapData[$key][$model]['path']= $pth."/".$path;
	            }
	             $sitemapData[$key][$model]['type'] =(isset($d[$model]['type']))?$d[$model]['type']:$model;
	            $sitemapData[$key][$model]['updated'] = $d[$model]['updated'];
	            if (isset($d['Seo']))
	            		$sitemapData[$key][$model]['Seo'] = $d['Seo'];   
	            else {
	            	if (isset($d[$model]['description']))
	            		$sitemapData[$key][$model]['Seo']['meta_description']=$d[$model]['description'];
	            	$sitemapData[$key][$model]['Seo']['node_id'] =$d[$model]['id'];
	            	$sitemapData[$key][$model]['Seo']['id'] =1;
