 ...
 */
 ...
 $body = preg_replace_callback('/\[AlbumsGallery.*?\]/', array(&$this,'replaceForAlbumsGallery'), $body);
	...
	public function replaceForGallery($subject){
		...
		return $this->Layout->View->element('gallery', array('plugin' => 'photon', 'gallery_id' => $this->gallery_id++,
		'cache' => array( 'time'=> '+999 day',
                        'key' => $this->gallery_id,
                        'config' => 'albums',
                    )));
	}
public function replaceGallerydiv($slug){
		return $this->Layout->View->element('gallery', array('plugin' => 'photon', 'slug' => $slug,
		'cache' => array( 'time'=> '+999 day',
                        'key' => $slug,
                        'config' => 'albums',
                    )));
	...
	public function replaceForAlbumsGallery($subject){
		//preg_match('/\[Gallery:(.*?)\]/', $subject[0], $matches);
		return $this->Layout->View->element('albumsgallery', array('plugin' => 'photon', 'gallery_id' => $this->gallery_id++,
		'cache' => array(
		'time'=> '+999 day',
                        'key' => $this->gallery_id,
                        'config' => 'albums',
